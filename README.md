...
> ⚠️ **GitHub OAuth Redirect URI**
>
> When you register your OAuth application on GitHub, set the
> **Authorization callback URL** to match your backend endpoint. For local
> development, use one of:
>
> ```
> http://localhost:8000/github/callback
> # or, if using the API prefix:
> http://localhost:8000/api/v1/github_auth/github/callback
> ```
>
> This project provides both routes; the first is exposed without any
> prefix for convenience (the second lives under `/api/v1`).  Make sure the
> value you put in `.env` (settings.REDIRECT_URI) exactly matches the
> one configured on GitHub, otherwise the provider will reject the request.
...
1. **Configure Alembic** in `alembic.ini`:
   ```ini
   sqlalchemy.url = postgresql://your_username:your_password@localhost:6432/DotDev
   # OR for SQLite:
   # sqlalchemy.url = sqlite:///DotDev.db
   ```
